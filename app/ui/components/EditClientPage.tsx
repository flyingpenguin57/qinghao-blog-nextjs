...
import getTagIdByName, { tags } from '@/app/lib/constants';
...
import { Article } from '@/app/lib/db-definition';

interface EditPageProps {
  article: Article | null;
}

export default function EditClientPage({ article }: EditPageProps) {
  const [text, setText] = useState(article?.content || ""); // 保存 textarea 的内容
  ...
  const [tag, setTag] = useState(article ? {id: getTagIdByName(article.tags[0]), name: article.tags[0]} : tags[0])
  ...
  const saveArticle = async (status: number) => {
    ...
    const res = await fetch('api/article/save', {
      ...
      body: JSON.stringify({ id: article?.id, content: text, tags: [tag.name], status }), 
    })
    setOnSaving(false)
